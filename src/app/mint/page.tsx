  ...
  const steps = [
    ...
    },
    {
      ...
      detail: request.travelRuleData
        ? `Originator: ${request.travelRuleData.originatorName} · LEI: ${request.travelRuleData.originatorLEI}`
  ...
  return (
    <>
      <tr
        ...
        onClick={onExpand}
        style={{ background: expanded ? "var(--bg-hover)" : undefined }}
      >
        <td>
          <span
                ...
                <div>
                  ...
                  {request.travelRuleData ? (
                    <div
                      ...
                      style={{ background: "var(--bg-base)", border: "1px solid var(--border)", fontFamily: "var(--font-mono)" }}
                    >
                      {Object.entries(request.travelRuleData)
                        .filter(([k]) => !["timestamp"].includes(k))
                        .map(([k, v]) => (
                          <div key={k} className="flex gap-2">
                            <span style={{ color: "var(--text-tertiary)", minWidth: "160px", flexShrink: 0 }}>
                              {k}
                            </span>
                  ...
                  {request.amlScreening ? (
                    ...
                    >
                      {Object.entries(request.amlScreening)
                        ...
                        .map(([k, v]) => (
                          <div key={k} className="flex gap-2">
                            <span style={{ color: "var(--text-tertiary)", minWidth: "120px", flexShrink: 0 }}>
                              {k}
                            </span>
...
export default function MintPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [coinsRes, reqsRes] = await Promise.all([
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const newRequests = await reqsRes.json();
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {loading
                    ...
                    : stablecoins.map((coin) => (
                        <button
                          key={coin.symbol}
                          type="button"
                          onClick={() => setForm((f) => ({ ...f, stablecoinSymbol: coin.symbol }))}
                          className="py-2 px-3 rounded-lg text-sm font-medium transition-all duration-150"
                          style={{
                            background:
                              form.stablecoinSymbol === coin.symbol
                                ? "var(--accent-subtle)"
                                : "var(--bg-elevated)",
                            border: `1px solid ${
                              form.stablecoinSymbol === coin.symbol
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full py-2.5 pl-6 pr-3 rounded-lg text-sm outline-none transition-all"
                    style={{
                      background: "var(--bg-elevated)",
                      border: "1px solid var(--border)",
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, destinationWallet: e.target.value }))}
              ...
              {[
                ...
                <div key={k} className="flex justify-between gap-2">
                  ...
                  <span
                    ...
                    style={{
                      color: v === "VERIFIED" || v === "CLEAR" ? "var(--success)" : "var(--text-secondary)",
                      fontFamily: "var(--font-mono)",
                    }}
                  >
                    {v}
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {error && (
            <div
              ...
              style={{ background: "var(--error-dim)", border: "1px solid rgba(244,63,94,0.3)" }}
            >
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold" style={{ color: "var(--success)", fontFamily: "var(--font-display)" }}>
                    Mint Successful
                  </p>
